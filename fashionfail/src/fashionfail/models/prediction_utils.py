...
def _filter_preds_for_classes(
    ...
    filtered_boxes = (
        filtered_boxes.reshape(
            (0, 4)) if filtered_boxes.size == 0 else filtered_boxes
    )
...
def clean_df_preds(
    ...
    columns_to_keep = ["image_file", "classes", "scores", "boxes"]
    if "masks" in df_preds.columns:
        columns_to_keep.append("masks")
    ...
    if filter_cat_ids:
        filtered_preds = df_preds.apply(
            _filter_preds_for_classes, axis=1, class_ids=filter_cat_ids)
        df_preds[["classes", "scores", "boxes"]] = zip(*filtered_preds)
        if "masks" in df_preds.columns:
            df_preds["masks"] = [pred[3] for pred in filtered_preds]
...
def _fix_class_ids(series):
    ...
    new_id_to_org_id = dict(
        zip(df_mapping["new_cat_id"], df_mapping["old_cat_id"]))
...
def convert_preds_to_coco(
    ...
    output_json_file = preds_path.replace(
    ...
    explode_columns = ["classes", "scores", "boxes"]

    # if "masks" in df_preds.columns:
    #     explode_columns.append("masks")
    df_exploded = df_preds.explode(explode_columns)
    ...
    # if "masks" in df_exploded.columns:
    #     rename_columns["masks"] = "mask"
    df_exploded = df_exploded.rename(columns=rename_columns)
    ...
    # if "mask" in df_exploded.columns:
    #     df_exploded["mask"] = df_exploded["mask"].apply(
    #         lambda x: {"size": x["size"], "counts": x["counts"].decode(
    #             "utf-8")} if pd.notna(x) else None
    #     )

    # Sort predictions by score
    df_exploded = df_exploded.sort_values(
        ["image_file", "score"], ascending=False)
    ...
    logger.debug(
        f"ANNOTATION PATH ----------------------- {anns_path}, {check}")
    for _, row in df_exploded.iterrows():
        # retrieve COCO imgId
        # Find cause of this bug.
        if coco_imgs[coco_imgs.file_name == row["image_file"]].empty:
            continue
        image_id = coco_imgs[coco_imgs.file_name ==
                             row["image_file"]].id.values[0]
        ...
        # if "mask" in df_exploded.columns and pd.notna(row["mask"]):
        #     entry["segmentation"] = row["mask"]

        json_data.append(entry)

    logger.info(f"Number of predictions to save: {len(json_data)}")
    if len(json_data) == 0:
        logger.warning(
            f"No predictions to save for {preds_path}. The output file will be empty and COCO evaluation will fail.")

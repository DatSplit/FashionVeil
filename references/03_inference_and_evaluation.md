...
> [!IMPORTANT]
> The FashionFail repository is utilized for inferencing and evaluation, credits to [Velioglu et al.](https://github.com/rizavelioglu/fashionfail)

For inference on the benchmark datasets, we utilize a modified version of the [FashionFail repository](https://github.com/rizavelioglu/fashionfail/tree/main).

We compare `RT-DETR`, `RF-DETR-B`, and `RF-DETR-L` with the state-of-the-art FashionFormer model, and with the Attribute Mask R-CNN (`amrcnn`) and Facere models on the `Fashionpedia` and `FashionVeil` datasets.

Table below is partially modified from [FashionFail - Inference](https://github.com/rizavelioglu/fashionfail/blob/main/references/04_inference.md?plain=1).
...
| `RT-DETR`             | `RT-DETR` model trained on `FashionFail-train`.                                         | ResNet50 / ResNet101 | available on request                                    |
| `RF-DETR-B`           | `RF-DETR-B` model trained on `FashionFail-train`.                                       |        DinoV2        | available on request                                    |
| `RF-DETR-L`           | `RF-DETR-L` model trained on `FashionFail-train`.                                       |        DinoV2        | available on request                                    |
| `RF-DETR-L-finetuned` | `RF-DETR-L` model trained on `FashionFail-train` and fine-tuned on `FashionVeil-train`. |        DinoV2        | available on request                                    |


For fair evaluation we make predictions on the same images as done in the [FashionFail paper](https://arxiv.org/abs/2404.08582).

This dataset can be constructed by running:
```bash
cd fashionfail/src
python3 fashionfail/data/make_fashionpedia.py
```
This will create a folder in your `~/user/.cache` directory. The exact same `train/val` split is used when downloading our [Fashionpedia for RF-DETR dataset](01_dataset_downloads.md#fashionpedia-for-rf-detr).
...
python fashionfail/models/predict_rtdetr.py \
    --model_name "rtdetr" \
    --model_path "PATH_TO_YOUR_ONNX_MODEL" \
    --image_dir "DIRECTORY_OF_FASHIONPEDIA_TEST_IMAGES" \
    --out_dir "predictions_fashionpedia_test_rtdetr_ResNet50/"
```
* `image_dir` is usually located at:  **~/user/.cache/fashionpedia/images/test**.
...
```
* `preds_path` is usually located at **~/SOME_DIRECTORIES/predictions_fashionpedia_test_rtdetr_ResNet50/rtdetr.npz**.
* `anns_path` is usually located at **~/user/.cache/fashionpedia/instances_attributes_val2020.json**.
...
python fashionfail/models/predict_rfdetr.py \
    ...
    --model_name "rfdetr" \
    --image_dir "DIRECTORY_OF_FASHIONPEDIA_TEST_IMAGES" \
    --out_dir "predictions_fashionpedia_test_rfdetrb/"
```
Now the predictions (`preds_path`) can be evaluated based on the ground truth bounding boxes and class labels (`anns_path`) on the 5 metrics mentioned in the report on the `fashionpedia-test` benchmark dataset.
...
RF-DETR-L was also fine-tuned on `FashionVeil-train/val` and evaluated on `FashionVeil-test` for all occlusion levels, which is shown in Table 7.4 and Figure 7.2:
